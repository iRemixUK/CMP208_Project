...
extern const SceGxmProgram _binary_default_3d_shader_v_gxp_start;
extern const SceGxmProgram _binary_default_3d_shader_f_gxp_start;

namespace gef
{
	Default3DShaderVita::Default3DShaderVita(SceGxmShaderPatcher* shader_patcher, const Default3DShaderData* const shader_data) :
			ShaderVita(shader_patcher, &_binary_default_3d_shader_v_gxp_start, &_binary_default_3d_shader_f_gxp_start),
//			vertex_shader_parameters_(NULL),
//			fragment_shader_parameters_(NULL),
			shader_data_(shader_data)

	{
		const SceGxmProgram *const vertex_program	= &_binary_default_3d_shader_v_gxp_start;
		const SceGxmProgram *const fragment_program	= &_binary_default_3d_shader_f_gxp_start;
		...
		SceGxmVertexAttribute vertex_attributes[3];
		...
		vertex_attributes[2].format = SCE_GXM_ATTRIBUTE_FORMAT_F32;
		vertex_attributes[2].componentCount = 2;
		vertex_attributes[2].regIndex = sceGxmProgramParameterGetResourceIndex(vertex_param_input_uv_attribute);


		SceGxmVertexStream vertex_streams[1];
		vertex_streams[0].stride = sizeof(gef::Mesh::Vertex);
	...
	Default3DShaderVita::~Default3DShaderVita()
	{
	}

	void Default3DShaderVita::SetConstantBuffers(SceGxmContext* context, const void* data)
	{
...
//		vertex_shader_data_.wvp=renderer->projection_matrix()*renderer->view_matrix()*renderer->world_matrix();
		...
		sceGxmSetUniformDataF(fragment_shader_data_buffer, fragment_shader_parameters_ambient_light_colour_, 0, 4, (const float *)&fragment_shader_data_.ambient_light_colour);
		sceGxmSetUniformDataF(fragment_shader_data_buffer, fragment_shader_parameters_light_colour_, 0, 16, (const float *)&fragment_shader_data_.light_colour[0]);
	}
}